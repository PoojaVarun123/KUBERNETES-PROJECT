...
Answer:
AWS Lambda is a serverless compute service that allows you to run code without provisioning or managing servers. You simply upload your code, and Lambda takes care of everything required to run and scale your application.
...
Answer:
AWS Lambda automatically scales horizontally by spawning multiple instances of the function in parallel, depending on the number of incoming requests. It can handle thousands of concurrent executions without manual configuration.
...
🔹 SECTION 2: SCENARIO-BASED LAMBDA INTERVIEW QUESTIONS
