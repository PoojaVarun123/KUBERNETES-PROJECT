Q1. What is Amazon CloudWatch?
      Amazon CloudWatch is a fully managed monitoring and observability service provided by AWS. It collects monitoring and operational 
      data in the form of logs, metrics, and events. It allows you to monitor AWS resources like EC2, RDS, Lambda, ECS, etc., and 
      on-premises applications, set alarms, and respond automatically to changes.
      Key Features:
                  -Real-time metrics
...
Q3. What is a CloudWatch Alarm?
      A CloudWatch Alarm watches a single metric or a math expression. When the value exceeds a threshold for a specified number of evaluation 
      periods, the alarm performs actions such as:
...
Q4. What is CloudWatch Logs?
            CloudWatch Logs helps you monitor, store, and access log files from AWS resources (like EC2, Lambda, API Gateway) or custom logs. 
            You can:
