...
🔹 2. Scenario-Based CloudWatch Interview Questions and Answers
...
🔹 3. High-Level Architecture and Integration Questions
...
Trigger Lambda on log pattern match (via Subscription filter)

CloudWatch Events → Lambda:

On schedule (cron) or AWS service state changes

CloudWatch Alarm → Lambda:

Trigger Lambda when an alarm breaches (e.g., restart service)
...
Theoretical	Metrics, Alarms, Logs, Dashboards, Differences with CloudTrail
Scenario-Based	CPU issues, Auto healing, Exceptions, ECS monitoring, Latency investigations
Architecture	High-level observability setup, Cross-account logging, Real-time alerting
